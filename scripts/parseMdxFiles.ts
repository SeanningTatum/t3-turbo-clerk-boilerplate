...
async function parseMdxFiles(
  ...
  try {
    ...
    const mdxFilePaths = await getAllMdxFiles(absoluteFolderPath);
    ...
    combinedContent += `=== MDX Content Compilation ===\n`;
    ...
    console.log(
      `\nSuccessfully combined ${mdxFilePaths.length} MDX files into ${outputPath}`,
    );
  } catch (error) {
...
async function main() {
  try {
    ...
    await parseMdxFiles(
      "./apps/docs/pages",
      "./apps/docs/app/api/chat/combined_content.txt",
    );
  } catch (error) {
